...
func _process(delta: float) -> void:
	if plate_state:
		update_visuals_on(delta * 3.0)
	else:
		update_visuals_off(delta * 3.0)
...
func update_visuals_on(lerp_alpha: float) -> void:
	glow.set_shader_parameter("intensity", lerp(glow.get_shader_parameter("intensity"), 3.0, lerp_alpha / 2.0))
	button.scale.y = lerp(button.scale.y, 0.2 if plate_state else 1.0, lerp_alpha)

func update_visuals_off(lerp_alpha: float) -> void:
	glow.set_shader_parameter("intensity", lerp(glow.get_shader_parameter("intensity"), -3.0, lerp_alpha / 2.0))
	button.scale.y = lerp(button.scale.y, 0.2 if plate_state else 1.0, lerp_alpha)
